...
.element_<?php echo $galleryID; ?> {
    max-width:<?php echo get_option('ht_view6_width')+2*get_option('ht_view6_border_width'); ?>px;
    margin:0 2px 10px 0;
    border:<?php echo get_option('ht_view6_border_width'); ?>px solid #<?php echo get_option('ht_view6_border_color'); ?>;
    border-radius:<?php echo get_option('ht_view6_border_radius'); ?>px;
    outline:none;
    overflow:hidden;
}
.element_<?php echo $galleryID; ?> .image-block_<?php echo $galleryID; ?> {
    position:relative;
    width: 100%;
	width:<?php echo get_option('ht_view6_width'); ?>px;
    max-width:<?php echo get_option('ht_view6_width'); ?>px;
}
...
.element_<?php echo $galleryID; ?> .image-block_<?php echo $galleryID; ?> img {
    width: 100%;
    max-width:<?php echo get_option('ht_view6_width'); ?>px !important;
    height:auto;
    display:block;
...
.element_<?php echo $galleryID; ?> .title-block_<?php echo $galleryID; ?> a, .element_<?php echo $galleryID; ?> .title-block_<?php echo $galleryID; ?> a:link, .element_<?php echo $galleryID; ?> .title-block_<?php echo $galleryID; ?> a:visited, .element_<?php echo $galleryID; ?> .title-block_<?php echo $galleryID; ?> {
    ...
    white-space:nowrap;
    z-index:20;
    font-size: <?php echo get_option("ht_view6_title_font_size");?>px;
    color:#<?php echo get_option("ht_view6_title_font_color");?>;
    font-weight:normal;
}
...
.element_<?php echo $galleryID; ?> .title-block_<?php echo $galleryID; ?> {
    ...
    width:100%;
    padding-top:5px;
    height: <?php echo 20+get_option('ht_view6_title_font_size'); ?>px;
    bottom:-41px;
    background: <?php
			list($r,$g,$b) = array_map('hexdec',str_split(get_option('ht_view6_title_background_color'),2));
				$titleopacity=get_option("ht_view6_title_background_transparency")/100;
...
.element_<?php echo $galleryID; ?> .title-block_<?php echo $galleryID; ?> a:hover, .element_<?php echo $galleryID; ?> .title-block_<?php echo $galleryID; ?> a:focus, .element_<?php echo $galleryID; ?> .title-block_<?php echo $galleryID; ?> a:active {
    color:#<?php echo get_option("ht_view6_title_font_hover_color");?>;
    text-decoration:none;
}
.load_more4 {
    margin: 10px 0;
    position:relative;
    text-align:<?php if(get_option('video_ht_view4_loadmore_position') == 'left') {echo 'left';}
			elseif (get_option('video_ht_view4_loadmore_position') == 'center') { echo 'center'; }
			elseif(get_option('video_ht_view4_loadmore_position') == 'right') { echo 'right'; }?>;
    width:100%;
}
.load_more_button4 {
    border-radius: 10px;
    ...
    padding:5px 15px;
    font-size:<?php echo get_option('video_ht_view4_loadmore_fontsize'); ?>px !important;;
    color:<?php echo '#'.get_option('video_ht_view4_loadmore_font_color'); ?> !important;;
    background:<?php echo '#'.get_option('video_ht_view4_button_color'); ?> !important;
    cursor:pointer;
}
.load_more_button4:hover{
    color:<?php echo '#'.get_option('video_ht_view4_loadmore_font_color_hover'); ?> !important;
    background:<?php echo '#'.get_option('video_ht_view4_button_color_hover'); ?> !important;
}
.loading4 {
    display:none;
}
.paginate4{
    font-size:<?php echo get_option('video_ht_view4_paginator_fontsize'); ?>px !important;
    color:<?php echo '#'.get_option('video_ht_view4_paginator_color'); ?> !important;
    text-align: <?php echo get_option('video_ht_view4_paginator_position'); ?>;
    margin-top: 25px;
}
.paginate4 a{
...
.icon-style4{
    font-size: <?php echo get_option('video_ht_view4_paginator_icon_size'); ?>px !important;
    color:<?php echo '#'.get_option('video_ht_view4_paginator_icon_color'); ?> !important;
}
.clear{
    clear:both;
}
...
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?>{
    ...
    right:0;
    z-index: 3;
    color: #<?php echo get_option('ht_lightbox_likedislike_font_color'); ?>;
    display: none;
}
...
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?> .huge_it_gallery_dislike_wrapper {
    position:relative;
    background: <?php
			list($r,$g,$b) = array_map('hexdec',str_split(get_option('ht_lightbox_likedislike_bg'),2));
				$titleopacity=get_option("ht_lightbox_likedislike_bg_trans")/100;
				echo 'rgba('.$r.','.$g.','.$b.','.$titleopacity.')  !important';
	?>;
    display: inline-block;
    border-radius: 3px;
...
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?> .like_thumb_up{
    ...
    top: 5px;
    left: 4px;
    color:#<?php echo get_option('ht_lightbox_likedislike_thumb_color')?>;
}
...
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?> .like_thumb_active{
    color: #<?php echo get_option('ht_lightbox_likedislike_thumb_active_color')?> !important;
}
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?> .like_font_active{
    color: #<?php echo get_option('ht_lightbox_active_font_color')?> !important;
}
...
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?> .huge_it_like .huge_it_like_thumb {
    ...
    float: left;
<?php $heartCount='';
if(get_option('ht_lightbox_rating_count')=='off'){
    $heartCount="transparent";
}else{
    $heartCount='#'.get_option('ht_lightbox_likedislike_font_color');
}
?>
...
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?> .huge_it_like  .likeheart{
    font-size: 32px;
    color:#<?php echo get_option('ht_lightbox_heart_likedislike_thumb_color')?>;
    position: absolute;
    top: 4px;
    left: 3px;
...
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?> .like_thumb_active{
    color: #<?php echo get_option('ht_lightbox_heart_likedislike_thumb_active_color')?> !important;
}
.huge_it_gallery_like_cont_<?php echo $galleryID.$pID; ?> .like_font_active{
<?php if(get_option('ht_lightbox_rating_count')!='off'):?>
    color: #<?php echo get_option('ht_lightbox_active_font_color')?> !important;
<?php endif; ?>
}
@media screen and (min-width: 768px){
